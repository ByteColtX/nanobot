...
_AGENT_EMPTY_RESPONSE_FALLBACK = "I completed the tool steps but couldn't produce a final answer. Please try again or narrow the task."
...
class OutboundSender:
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if self._should_suppress_llm_connection_error(msg):
            ...
            return
        if self._should_suppress_llm_server_error(msg):
            logger.warning(
                "NapCat [SEND] suppressed LLM error response chat_id={} preview={!r}",
                msg.chat_id,
                _coerce_str(msg.content).strip()[:160],
            )
            return
    ...
    @staticmethod
    def _should_suppress_llm_server_error(msg: OutboundMessage) -> bool:
        """拦截底层 LLM 错误响应（所有以 'Error: ' 开头的消息），避免将内部异常直接发到聊天侧。"""
        if msg.media:
            return False
